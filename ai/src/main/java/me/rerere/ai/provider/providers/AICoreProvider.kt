...
class AICoreProvider(private val context: Context) : Provider<ProviderSetting.AICore> {
    ...
    override suspend fun streamText(
        ...
        // foreground (otherwise throws ErrorCode 30). Bring our own UI to the foreground
        // before inference so the request can proceed. Best-effort: if the OS refuses
        // (locked screen or recent BAL restrictions), the call surfaces the translated error.
     ...
     * detect that we're backgrounded, fire the launcher Intent for our own package and poll until the
...
private fun buildAiCoreMiniSystemPrefix(tools: List<Tool>): String = buildString {
    ...
    if (tools.isNotEmpty()) {
        ...
        appendLine("Example: <tool_call>{\"name\":\"workspace_read_file\",\"input\":{\"path\":\"notes.txt\"}}</tool_call>")
        // Generalised "stop after success" rule. Previously only named launch_app/open_url
        // — Nano then looped on long-running tools, re-emitting the
...
private class ToolTagParser(private val tools: List<Tool> = emptyList()) {
     ...
     * back to "command" for legacy single-argument tool schemas.
     */
