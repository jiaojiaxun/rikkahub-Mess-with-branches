...
class RouteActivity : ComponentActivity() {
    ...
    fun AppRoutes() {
        ...
        val startupProgress by AppStartupProgressTracker.state.collectAsStateWithLifecycle()
                    ...
                    AnimatedVisibility(
                        ...
                        Box(
                            ...
                            Column(
                                ...
                                RabbitLoadingIndicator(modifier = Modifier.size(92.dp))
                                Text(
                                    text = when (startupProgress.stage) {
                                        AppStartupStage.Starting -> "正在启动"
                                        AppStartupStage.LoadingCore -> "正在加载核心服务"
                                        AppStartupStage.PreparingData -> "正在准备数据"
                                        AppStartupStage.LoadingServices -> "正在加载服务"
                                        AppStartupStage.Finalizing -> "正在完成初始化"
                                        AppStartupStage.Ready -> "准备就绪"
                                    },
                                    style = MaterialTheme.typography.titleMedium,
                                )
                                LinearProgressIndicator(
                                    progress = { startupProgress.percent / 100f },
                                    modifier = Modifier.fillMaxWidth(),
                                )
                                Text(
                                    text = "${startupProgress.percent}%",
