...
// ToolRuntimeLimits.turnBudgetMs (default 10 min); every read site below uses that holder.
...
class GenerationHandler(
    ...
    fun generateText(
        ...
        // Optional per-call addendum appended to the system prompt. Used by the active
        // in-app surface when the model needs runtime context without polluting the user
        // message body — without this the
        ...
        for (stepIndex in 0 until maxSteps) {
            ...
            if (pendingTools.isEmpty()) {
                ...
                for (tool in tools) {
                    ...
                    // previously approved tools still cannot smuggle one of these
                    // patterns through. Result: the tool is marked Denied with the hardline
                ...
                when (tool.approvalState) {
                    ...
                    is ToolApprovalState.Pending -> {
                                                ...
                                                put(
                                                    "recovery", JsonPrimitive(
                                                        "You have already called ${tool.toolName} with identical arguments " +
                                                            ...
                                                            "X in the browser' use open_url(\"https://www.google.com/search?q=X\") " +
                                                            "instead of repeatedly fighting the browser URL bar via set_text."
                                                    )
                                                )
                                            }
                                        )
                        ...
                        runCatching {
                            ...
                            // Keep oversized tool results bounded in memory and in the model
                            // context. The compact build does not expose a shell-backed output
                            // directory, so the result is summarized instead of pointing to one.
    ...
    private suspend fun generateInternal(
        ...
        if (stream) {
            ...
            processingStatus.value = "正在接收流式回复"
        ...
        return listOf(
            UIMessagePart.Text(
                buildString {
                    ...
                    appendLine("Only a preview is included to protect the context window.")
                    appendLine()
                    append(preview)
                }
            )
