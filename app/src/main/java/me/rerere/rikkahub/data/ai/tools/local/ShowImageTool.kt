...
/**
 * `show_image` — display an image file inline in the in-app chat.
 *
 ...
 *
 * No approval required — reading a file already available to the file tools does not
 * expand access beyond the user-approved workspace and file paths.
 *
 * [modelCanSeeImages] gates what the LLM is told in the result envelope. The
 * `UIMessagePart.Image` is always returned for the user's in-app display. But when the
 * active model has no image input modality, the text
...
fun showImageTool(
    ...
    name = "show_image",
    description = "Display an image file inline in the in-app chat. Use when the user wants to see an image at a known workspace or user-visible path. Path accepts ~ or absolute. Supported: PNG, JPEG, WebP, GIF, BMP.",
    parameters = {
        InputSchema.Obj(
    ...
    if (!modelCanSeeImages) {
        ...
        put(
            ...
            "The image is now displayed to the user, but the current model has no " +
                ...
                "guess what the image shows. To read it, OCR or otherwise process the file at " +
                "`path` with the available image/OCR tools.",
        )
    }
}.toString()
