 ...
 */
private fun decodeUuidSet(raw: String?): Set<Uuid> = raw?.let {
    runCatching {
        JsonInstant.decodeFromString<Set<String>>(it)
            .mapNotNull { value -> runCatching { Uuid.parse(value) }.getOrNull() }
            .toSet()
    }.getOrElse {
        Log.w(TAG, "Failed to decode deleted-id set, using empty set", it)
        emptySet()
    }
} ?: emptySet()
...
class SettingsStore(
    ...
    companion object {
        ...
        val DELETED_PROVIDER_IDS = stringPreferencesKey("deleted_provider_ids")

        // 助手
        val SELECT_ASSISTANT = stringPreferencesKey("select_assistant")
        val ASSISTANTS = stringPreferencesKey("assistants")
        val DELETED_ASSISTANT_IDS = stringPreferencesKey("deleted_assistant_ids")
        ...
        val DELETED_MCP_SERVER_IDS = stringPreferencesKey("deleted_mcp_server_ids")
    ...
    val settingsFlowRaw = dataStore.data
        .catch { exception ->
            ...
            Settings(
                ...
                deletedBuiltInProviderIds = decodeUuidSet(preferences[DELETED_BUILTIN_PROVIDER_IDS]),
                deletedProviderIds = decodeUuidSet(preferences[DELETED_PROVIDER_IDS]),
                ...
                deletedAssistantIds = decodeUuidSet(preferences[DELETED_ASSISTANT_IDS]),
                ...
                deletedMcpServerIds = decodeUuidSet(preferences[DELETED_MCP_SERVER_IDS]),
    ...
    private suspend fun updateInternal(settings: Settings) {
        dataStore.edit { preferences ->
            ...
            preferences[DELETED_PROVIDER_IDS] = JsonInstant.encodeToString(
                settings.deletedProviderIds.map { it.toString() }.toSet()
            )

            preferences[ASSISTANTS] = JsonInstant.encodeToString(settings.assistants)
            preferences[DELETED_ASSISTANT_IDS] = JsonInstant.encodeToString(
                settings.deletedAssistantIds.map { it.toString() }.toSet()
            )
            ...
            preferences[DELETED_MCP_SERVER_IDS] = JsonInstant.encodeToString(
                settings.deletedMcpServerIds.map { it.toString() }.toSet()
            )
            preferences.remove(stringPreferencesKey("sub_agents"))
...
data class Settings(
    ...
    /** IDs explicitly removed by the user; retained so backup merge can honor deletions. */
    val deletedProviderIds: Set<Uuid> = emptySet(),
    val assistants: List<Assistant> = DEFAULT_ASSISTANTS,
    val deletedAssistantIds: Set<Uuid> = emptySet(),
    /**
    ...
    val deletedMcpServerIds: Set<Uuid> = emptySet(),
