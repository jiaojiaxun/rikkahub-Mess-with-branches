...
private const val DEFAULT_COPY_BUFFER_SIZE = 16 * 1024
...
class WebDavClient(
    ...
    suspend fun put(
        ...
        runCatching {
            ...
            val response: HttpResponse = httpClient.request(url) {
                ...
                // Stream file content to avoid loading the whole backup zip into heap while
                // reporting the bytes actually written to Ktor's request channel.
                setBody(object : OutgoingContent.WriteChannelContent() {
                    override val contentType = contentType.let { io.ktor.http.ContentType.parse(it) }
    ...
    /**
     * Verify that a successful PUT is visible through the server's read/list path.
     * Some WebDAV providers acknowledge PUT before their directory index is refreshed;
     * treating the HTTP PUT response alone as completion caused false-success backups.
     */
    suspend fun verifyUploaded(path: String, expectedSize: Long): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            val headInfo = head(path).getOrNull()
            val remoteInfo = headInfo ?: list().getOrThrow().firstOrNull { resource ->
                resource.displayName == path.substringAfterLast('/') ||
                    resource.href.trimEnd('/').substringAfterLast('/') == path.substringAfterLast('/')
            } ?: throw WebDavException(
                "Uploaded file is not visible on the WebDAV server: $path",
                404,
                "PUT returned success, but HEAD/LIST could not find the resource",
            )

            // A few providers omit Content-Length on HEAD/PROPFIND. A reported non-zero
            // length must still agree with the local ZIP, otherwise the upload is incomplete.
            if (remoteInfo.contentLength > 0L && remoteInfo.contentLength != expectedSize) {
                throw WebDavException(
                    "Remote backup size mismatch for $path: expected $expectedSize, " +
                        "found ${remoteInfo.contentLength}",
                    409,
                    "The server acknowledged PUT but stored a different byte count",
                )
            }
            Log.d(TAG, "verifyUploaded success: $path (${remoteInfo.contentLength} bytes reported)")
            Unit
        }
    }
