 ...
 * Mirrors the McpUrlGuard pattern from the URL-safety audit; this is always a user-driven action.
 *
...
class SkillUrlImporter(
     ...
     * authenticated or custom sources), they can compose existing tools to fetch the
     * content first and pass the body to this method:
     *
     *  - any authenticated fetch tool → pipe stdout into here
     *  - any local or remote fetch tool returning a markdown body
     ...
     * a source label, "clipboard", etc. It's persisted as `source-url:` in the saved
 ...
 *
 * Why: the source names are ordinary English words (Read, Write, Edit, Grep,
 * and similar labels). A global word-bounded replace over the whole body corrupted natural
...
object ToolNameTranscoder {
    private val Mappings: List<Pair<String, String>> = listOf(
        // openclaw → RikkaHub. Keys are case-sensitive; ordinary lower-case words are
        // intentionally not rewritten because they collide with prose.
        "Read" to "read_file",
