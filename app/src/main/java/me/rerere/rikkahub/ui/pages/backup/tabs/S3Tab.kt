...
fun S3Tab(
    ...
    Column(
        ...
        Column(
            ...
            progress?.let { S3ProgressCard(it) }
            BackupStatusCard(
    ...
    if (showRestoreMode) {
        AlertDialog(
            ...
            title = { Text("选择 S3 恢复方式") },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("合并会按单个对话判断新增或分叉；覆盖会在关闭 Room 后原子替换数据库。")
                    Text("当前文件：${pendingRestoreItem?.displayName.orEmpty()}")
                }
            },
            confirmButton = {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    TextButton(onClick = {
                        ...
                        pendingRestoreItem = null
                        showRestoreMode = false
                        if (item != null) scope.launch {
                            restoringItemId = item.displayName
                            runCatching {
                                vm.restoreFromS3(item, BackupRestoreMode.MERGE)
                                toaster.show(context.getString(R.string.backup_page_restore_success), type = ToastType.Success)
                                showBackupFiles = false
                                onShowRestartDialog()
                            }.onFailure { err ->
                                toaster.show(context.getString(R.string.backup_page_restore_failed, err.message ?: ""), type = ToastType.Error)
                            }
                    ...
                    Button(onClick = {
                        ...
                        if (item != null) scope.launch {
                            restoringItemId = item.displayName
                            runCatching {
                                vm.restoreFromS3(item, BackupRestoreMode.OVERWRITE)
                                toaster.show(context.getString(R.string.backup_page_restore_success), type = ToastType.Success)
                                showBackupFiles = false
                                onShowRestartDialog()
                            }.onFailure { err ->
            ...
            dismissButton = {
                TextButton(onClick = { pendingRestoreItem = null; showRestoreMode = false }) { Text("取消") }
            },
        )
    }
}
...
@Composable
private fun S3ProgressCard(progress: BackupProgress) {
    CardGroup {
        item(
            ...
            supportingContent = {
                Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    else LinearProgressIndicator(modifier = Modifier.fillMaxWidth(), progress = { fraction })
                    Text("${progress.percent ?: 0}% · ${progress.detail}")
                    if (progress.total > 0L) Text("${progress.completed} / ${progress.total} bytes")
                }
            },
        )
    }
