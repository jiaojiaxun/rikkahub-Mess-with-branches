...
private fun ChatPageContent(
    ...
    val sharedConversationImportLauncher = rememberLauncherForActivityResult(
        ...
        if (uri != null) {
            scope.launch {
                runCatching {
                    ...
                    vm.importSharedConversation(text)
                }.onSuccess { imported ->
                    toaster.show(
                        context.getString(R.string.chat_page_import_shared_success),
                        type = ToastType.Success,
                    )
                    navigateToChatPage(navController, chatId = imported.id)
                }.onFailure {
                    toaster.show(
                        context.getString(R.string.chat_page_import_shared_failed, it.message ?: ""),
    ...
    Surface(
        ...
        Scaffold(
            ...
            ChatList(
                ...
                onRetryOverwrite = { error ->
                    if (error.conversationId == conversation.id) vm.retryLastTurnOverwrite()
                },
                onContinueCustom = { error, customMessage ->
                    if (error.conversationId == conversation.id) {
                        vm.handleMessageSend(
                            content = listOf(UIMessagePart.Text(customMessage)),
                            answer = true,
                        )
                    }
                },
                ...
                onTranslate = { message, locale, mode ->
                    vm.translateMessage(message, locale, mode)
                },
...
private fun TopBar(
    ...
    onImportSharedConversation: () -> Unit,
) {
    val scope = rememberCoroutineScope()
    val toaster = LocalToaster.current
        ...
        actions = {
            ...
            Box {
                IconButton(
                    ...
                    Icon(HugeIcons.FileImport, stringResource(R.string.chat_page_import_shared_conversation))
                }
                DropdownMenu(
                    ...
                    DropdownMenuItem(
                        text = { Text(stringResource(R.string.chat_page_import_shared_conversation)) },
