...
class HardlineCommandGuardTest {
    ...
    // -----------------------------------------------------------------------
    // Tool-aware entrypoint: only current MCP/browser paths remain
    // -----------------------------------------------------------------------

    @Test fun `removed shell and automation tools are not gated or registered`() {
        for (toolName in listOf(
            ...
            assertNull(
                "$toolName must not have a live hardline tool branch",
                HardlineCommandGuard.checkTool(toolName, """{"command":"rm -rf /"}"""),
            )
        }
    }
    ...
    @Test fun `malformed JSON returns null instead of throwing`() {
        val reason = HardlineCommandGuard.checkTool("browser_click", "{not json")
